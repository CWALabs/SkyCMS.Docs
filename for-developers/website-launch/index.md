<!-- Audience: Backend and Full-Stack Developers -->
<!-- Type: Workflow -->
<!-- Status: Draft -->
<!-- Source: SkyCMS/Docs/Developer-Guides/ -->

# Website Launch Workflow (Migrated Guides)

These guides were migrated from the legacy `SkyCMS/Docs/Developer-Guides` set because they provide practical, end-to-end implementation workflows.

Use this section for a structured phase-based launch path.

## Start Here

- [Website Launch Workflow Guide](./Website-Launch-Workflow.md)
- [Creating Editable Areas](./CreatingEditableAreas.md)

## Workflow Phases

1. [Phase 1: Design and Planning](./01-Design-and-Planning.md)
2. [Phase 2: Creating Layouts](./02-Creating-Layouts.md)
3. [Phase 3: Creating Templates](./03-Creating-Templates.md)
4. [Phase 4: Building Home Page](./04-Building-Home-Page.md)
5. [Phase 5: Building Initial Pages](./05-Building-Initial-Pages.md)
6. [Phase 6: Preparing for Handoff](./06-Preparing-for-Handoff.md)

## Checklists

- [Pre-Launch Checklist](./Checklists/Pre-Launch-Checklist.md)
- [Content Creator Onboarding](./Checklists/Content-Creator-Onboarding.md)

## Related New-Set Docs

- [Developer Layouts Guide](../layouts.md)
- [Developer Templates Guide](../templates.md)
- [Layouts, Templates, and Articles](../layouts-templates-articles.md)
- [Site Builder Layouts Guide](../../for-site-builders/layouts.md)
