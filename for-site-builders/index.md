---
canonical_title: For Site Builders
description: Start here for layout, template, page, and widget implementation workflows in SkyCMS.
doc_type: Explanation
product_area: site-building
user_intent: find-site-builder-workflows-and-guides
audience:
	- Site Builders
	- Designers
	- Front-End Developers
difficulty: beginner
version: current
status: active
owner: docs-platform
last_reviewed: 2026-04-27
---

# For Site Builders

## Summary

Use this page when you need to:

- understand site builder responsibilities,
- find the right guide for layouts, templates, pages, or example implementations,
- navigate quickly to the supporting implementation guides.

## Role overview

SkyCMS content roles work together like this:

- Editors manage content operations and publishing.
- Site builders shape templates, layouts, and page composition.
- Developers implement deeper platform and integration changes.

In this section, a site builder is the person responsible for how content is structured and presented to end users.

Site builders typically:

- Design and maintain templates and reusable page structures.
- Define layout behavior and visual composition rules.
- Ensure pages are responsive, consistent, and brand-aligned.
- Collaborate with editors on authoring patterns and handoff.
- Coordinate with developers when implementation changes are required.

For role boundaries and permission details across all roles, see [Roles and permissions (RBAC)](../for-developers/roles-and-permissions.md).

## Start here

- [Layouts](./layouts.md)
- [Layout Examples Overview](./layout-examples/overview.md)
- [Templates](./templates.md)
- [Template Examples Overview](./template-examples/overview.md)
- [Pages](./pages.md)
- [Article Examples Overview](./article-examples/overview.md)
- [Widgets](./widgets/overview.md)
- [Style Guide Template](./style-guide-template.md)

Once you know the basics, [Site Builder Workflow Deep-Dive](./overview.md) walks the full planning-through-handoff process with a workflow diagram, design strategy, and quality gates.

## Verification

You are in the right workflow when you can:

- identify the right layout or template for a content goal,
- apply or update a page structure safely,
- hand off editor-ready regions with clear usage guidance.

## Related guides

- [For Editors](../for-editors/index.md)
- [For Developers](../for-developers/index.md)
